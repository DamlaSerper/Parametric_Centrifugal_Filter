...
// 1.Variables -- Belongs to cone but written as top cylinder due to being a copy file
a = 0.0065;
b = 0.0065;
c = 0.0020;
d = 0.0550;
...
r_cyl_top = a+c;
h_cyl_top = d-(b+f+a1);
cyl_top_lo_y = -(s_in+v+b+h_cyl_top); 
cyl_top_hi_y = -(s_in+v+b); 
r_cyl_bot = a3; 
...
Point(6) = {-r_cyl_bot, cyl_top_lo_y, 0, 1};
Point(7) = {0, cyl_top_lo_y, -r_cyl_bot, 1};
Point(8) = {r_cyl_bot, cyl_top_lo_y, 0, 1};
Point(9) = {0, cyl_top_lo_y, r_cyl_bot, 1};
...
ver_side = 13; 				// Number of nodes on the side wall of the cylinder in vertical direction
