...
// 1.Variables -- Belongs to bottom cylinder but written as top due to having a copy file
b = 0.0065;
d = 0.0550;
f = 0.0080;
v = 0.0087;
...
r_cyl_top = a3; // Radius of the bottom cylinder inside the inner basket, reference drawing (a3), unit: m
h_cyl_top = f; // Height of the bottom cylinder inside the inner basket, reference drawing (f), unit: m
cyl_top_lo_y = -(s_in+v+b+(d-b-f-a1)+f); // Reference drawing: -((v+d-f-a1) + s_in)
cyl_top_hi_y = -(s_in+v+b+(d-b-f-a1)); // Reference drawing:  -((v+d-a1) + s_in)
...
// Circles
hor_cir = 33; 				       // Number of nodes on the circle in horizontal direction
